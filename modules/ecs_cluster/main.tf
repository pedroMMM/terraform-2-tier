...
############################################
# The IAM Role for ECS services auto scaling
############################################
resource "aws_iam_role" "ecs_service_autoscale_role" {
  name = "${format("%s-cluster-ecs-service-scaling-role", var.application_name)}"
...
##################################################
# Managed IAM Policy for ECS services auto scaling
##################################################
resource "aws_iam_role_policy_attachment" "ecs_service_autoscale_policy" {
  role       = "${aws_iam_role.ecs_service_autoscale_role.name}"
